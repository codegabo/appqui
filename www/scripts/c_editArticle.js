...
function loadArticle() {
    ...
    $.ajax({
        ...
        success: function (datos) {
            $("#txtName").val(datos[0].nombre_art);
            $("#txtPrice").val(datos[0].precio_art);
            $("#txtDesc").val(datos[0].descri_art);
            $("#txtMark").val(datos[0].marcas_art);
            establecimiento_marcador_idPunto = datos[0].establecimiento_marcador_idPunto;
            if (datos[0].foto != null) {
                var image = document.getElementById('ProfileImage');
                image.src = "data:image/jpeg;base64," + datos[0].foto;
                b64str = datos[0].foto
            }
            setTimeout(function () {
                $(".modalLoading").hide();
        ...
        $.ajax({
            ...
            success: function (datos) {
                if (datos.CodigoResultado == '0000') {
                    ...
                    bootbox.alert({
                        message: "Articulo no editado, intentelo de nuevo",
                        callback: function () {
                        }
                    });
                    return false;
