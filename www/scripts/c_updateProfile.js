var urlServices = CONFIG.WS_URL;
var urlServices2 = CONFIG.WS_URL_MASK;
var token = CONFIG.TOKEN;

var idUsu = window.localStorage.getItem('IdUser');
var dataCities = [];
var user_genre;
var str;
var res;
var mode;
var optionsCities = '<option value="-1">SELECCIONAR CIUDAD</option>';
var b64str = "";

$(document).on('ready', onReady);

function onReady() {
var image = document.getElementById('ProfileImage');
image.src = "data:image/jpeg;base64," + b64str;

$('#btnRegister').on('click', register);
$('#CapturePhoto').on('click', capturePhoto);
$('#CaptureFromGallery').on('click', captureFromGallery);
$("#txtCelPhone").keyup(function() {
this.value = this.value.replace(/[^0-9\.]/g, '');
});
$("#txtIdent").keyup(function() {
this.value = this.value.replace(/[^0-9\.]/g, '');
});
$('#txtName').focus();

$.urlParam = function (name) {
var results = new RegExp('[\?&]' + name + '=([^&#]*)').exec(window.location.href);
return results[1] || 0;
};
mode = $.urlParam('Mode');
getCities();
if(mode=='E') {
getData();
}
loadSesion();

$('#sandbox-container input').datepicker({
format: "yyyy/mm/dd",
todayBtn: "linked",
autoclose: true,
todayHighlight: true,
language: "es",
ignoreReadonly: true
});


}

function loadSesion() {
if (active == 'true') {
var profilePic = window.localStorage.getItem('profilePic');
$("#profilePic").attr('src', 'data:image/png;base64,' + profilePic);
}
}

function isValidEmail(pEmail) {
var filter = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
if (!filter.test(pEmail)) {
return false;
}
return true;
}

function getCities() {
//Get Cities
$.ajax({
type: "GET",
crossDomain: true,
url: urlServices + 'Ciudades',
data: 'token=' + token,
success: function(datos) {
$.each(datos, function(i, Cities) {
// alert(Cities.Name);
optionsCities += '<option id="' + Cities.IdCity + '" value="' + Cities.Name + '">' + Cities.Name + '</option>';
dataCities[i] = Cities.Name;
});
$("#Cities").html(optionsCities);
$('input.typeahead').typeahead({
name: 'accounts',
local: dataCities
});
// $("#cboCities").html(optionsCities);
},
error: function(msg, url, line) {
alert('Ups, parece que te has quedado sin conexión a internet! Regresa pronto!!');
}
});
}

function getData() {
var loading_time = 1500;
$(".modalLoading").show();
$('#txtUser').prop("disabled", true);
$('#txtDate').prop("disabled", true);
$.ajax({
type: "GET",
crossDomain: true,
url: urlServices2 + '/GetUsuarioById',
data: 'token=' + token + '&idusu_usu=' + idUsu,
success: function(datos) {
if (datos.length != 0) {
$("#txtName").val(datos[0].nombre_usu);
$("#txtLastName").val(datos[0].apelli_usu);
$("#txtUser").val(datos[0].usuari_usu);
$("#txtEmail").val(datos[0].correo_usu);
$("#entity").val(datos[0].ciudad_usu.toUpperCase());
user_genre = datos[0].genero_usu; //data[2].color;
$('#selGenre').val(user_genre);
// $("#selGenre option[id='2']").attr("selected", "selected");
str = datos[0].fecNac_usu.replace(/-/g, '/');
res;
if (str.length <= 21) {
res = str.substring(0, 10);
} else {
res = str.substring(0, 10);
}
$('#txtDate').val(res);
// $("#txtDate").val(datos[0].fecNac_usu);
var imageB64 = "data:image/jpeg;base64," + datos[0].imagen_usu;
if (datos[0].imagen_usu != null) {
$('#ProfileImage').attr("src", imageB64);
}
}
setTimeout(function() {
$(".modalLoading").hide();
}, loading_time);
},
error: function(msg, url, line) {
setTimeout(function() {
$(".modalLoading").hide();
}, loading_time);
alert('Ups, parece que te has quedado sin conexión a internet! Regresa pronto!!');
}
});
}

function register() {
var loading_time = 2500;
$(".modalLoading").show();
setTimeout(function() {
$(".modalLoading").hide();
}, loading_time);
var txtName = $("#txtName").val();
var txtLastName = $("#txtLastName").val();
var txtUser = $("#txtUser").val();
var txtCity = $("#entity").val();
var selGenre = $("#selGenre option:selected").text();
var txtDate = $("#txtDate").val();
var txtPass = $("#txtPass").val();
var active = true;
var txtRol = 3;
var txtEmail = $("#txtEmail").val();
//VALIDATE Email
var validEmail = isValidEmail(txtEmail);
if (txtName == '') {
bootbox.alert({
message: "Nombre es campo obligatorio.",
callback: function() {
setTimeout(function() {
$('#txtName').focus();
}, 150);
}
});
return false;
} else if (txtLastName == '') {
bootbox.alert({
message: "Apellido es campo obligatorio.",
callback: function() {
setTimeout(function() {
$('#txtLastName').focus();
}, 150);
}
});
return false;
} else if (txtUser == '') {
bootbox.alert({
message: "Usuario es campo obligatorio.",
callback: function() {
setTimeout(function() {
$('#txtUser').focus();
}, 150);
}
});
return false;
} else if (txtEmail == '') {
bootbox.alert({
message: "Email es campo obligatorio.",
callback: function() {
setTimeout(function() {
$('#txtEmail').focus();
}, 150);
}
});
return false;
} else if (validEmail != true) {
bootbox.alert({
message: "Formato email no valido",
callback: function() {
setTimeout(function() {
$('#txtEmail').focus();
}, 150);
}
});
return false;
} else if (txtCity == '') {
bootbox.alert({
message: "Ciudad es campo obligatorio.",
callback: function() {
setTimeout(function() {
$('#txtCity').focus();
}, 150);
}
});
return false;
} else if (txtDate == '') {
bootbox.alert({
message: "Fecha es campo obligatorio.",
callback: function() {
setTimeout(function() {
$('#txtDate').focus();
}, 150);
}
});
return false;
} else {
$.ajax({
type: "PUT",
crossDomain: true,
contentType: "application/json; charset=utf-8",
url: urlServices + 'Usuarios',
data: '{"token":"' + token + '", "idusu_usu":"' + idUsu + '" , "correo_usu":"' + txtEmail + '", "usuari_usu":"' + txtUser + '", "nombre_usu":"' + txtName + '", "apelli_usu":"' + txtLastName + '", "contra_usu":"' + txtPass + '", "genero_usu":"' + selGenre + '", "imagen_usu":"' + b64str + '", "fecNac_usu":"' + txtDate + '", "ciudad_usu":"' + txtCity + '", "codrol_usu":"' + txtRol + '", "active":"' + active + '" }',
success: function(datos) {
if (datos.CodigoResultado == '0000') {
var idusu_usu = datos.MensajeResultado;
setTimeout(function() {
$(".modalLoading").hide();
}, loading_time);
bootbox.alert({
message: "Datos modificados correctamente.",
callback: function() {
setTimeout(function() {
window.location = 'perfil-usuario.html';
}, 150);
}
});
} else {
bootbox.alert({
message: "Numero identificacion o correo ya registrados en el sistema.",
callback: function() {}
});
return false;
}
},
error: function(msg, url, line) {
alert(line);
}
});
}
}

function capturePhoto() {
// Take picture using device camera and retrieve image as base64-encoded string
navigator.camera.getPicture(onSuccess, onFail, {
quality: 50,
destinationType: Camera.DestinationType.DATA_URL,
sourceType: Camera.PictureSourceType.CAMERA,
targetWidth: 50,
targetHeight: 50,
correctOrientation: true,
encodingType: Camera.EncodingType.JPEG
});
}

function captureFromGallery() {
navigator.camera.getPicture(onSuccess, onFail, {
quality: 50,
destinationType: Camera.DestinationType.DATA_URL,
sourceType: Camera.PictureSourceType.SAVEDPHOTOALBUM,
targetWidth: 50,
targetHeight: 50,
correctOrientation: true,
encodingType: Camera.EncodingType.JPEG
});
}

function onSuccess(imageData) {
var image = document.getElementById('ProfileImage');
image.src = "data:image/jpeg;base64," + imageData;
b64str = imageData;
// document.getElementById("divImg").innerHTML = imageData;
}

function onFail(message) {}

function goBack() {
window.history.back();
}
